...
- The owner dashboard still resolves directly with `200`.
...
- `HEAD` on `https://splashlens.com/api/partner-intake` still returned `404`, so keep treating that as method-specific drift rather than a current GET outage.
...
- The site partner-intake endpoint remains healthy on direct `GET`; only the `HEAD` probe is drifting.
