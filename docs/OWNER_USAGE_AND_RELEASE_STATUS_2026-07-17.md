...
- Public iOS truth remains on `1.0.6`.
...
- `https://app.splashlens.com/api/checkout?plan=monthly` returned direct `GET 302` with `X-SplashLens-Checkout-Mode: payment_link_direct`, and a full follow redirect landed on a live Stripe Checkout page with final HTTP `200`.
- `https://app.splashlens.com/api/checkout?plan=yearly` returned direct `GET 302` with `X-SplashLens-Checkout-Mode: payment_link_direct`, and a full follow redirect landed on a live Stripe Checkout page with final HTTP `200`.
...
- Gmail targeted stop-signal and recent-history checks found no new SplashLens-specific unsubscribe/remove request, complaint, or delivery-failure message after the prior completed run on 2026-07-16.
- A new warm reply arrived from Carolyn Dibrell at Service Industry News on 2026-07-16 (`19f6d449ab637b23`), and Joshua replied the same evening (`19f6d6d39a1e2d90`) to arrange a call after 9 AM Hawaii time.
- Same-day Gmail showed no new cold SplashLens outreach on 2026-07-17.
